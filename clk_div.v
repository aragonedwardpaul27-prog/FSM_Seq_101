...
============================================================================================
Description:
	This module takes a high-frequency input clock (clk_in) and it down to a
	lower frequency. By default, it divides a 50MHz input clock down to 1s clock period
	toggling the ouputs every 25,000,000 ticks
...
module clk_div(clk_out, clk_led, clk_in);
   ...
   //ports
    ...
    localparam integer TICKS  =  (FREQ_IN * PERIOD_OUT)/2; //Number of ticks every cycle
    ...
    always @(posedge clk_in) begin
        if(tick_cnt == (TICKS - 1)) begin 
            clk_out <= ~clk_out; //toggle out
            clk_led <= ~clk_led; // toggle led
            tick_cnt <= 28'b0; // reset tick counter
        end 
